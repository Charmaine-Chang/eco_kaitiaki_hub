"""Admin routes for bait station management: edit, retire, restore."""

from flask import render_template, request, redirect, url_for, flash, session, current_app
from PF_LU_APP.db import get_db, get_cursor, get_cursor_context
from PF_LU_APP.validators import validate_location_within_boundary
from PF_LU_APP.roles.admin import admin_bp
from PF_LU_APP.shared.permissions import get_equipment_status_id
from PF_LU_APP.shared.decorators import roles_required
from PF_LU_APP.constants import ROLE_SUPER_ADMIN, ROLE_COORDINATOR
from PF_LU_APP.traps.trap_repository import (
    fetch_storage_areas_scoped
)
from PF_LU_APP.traps.line_repository import (
    fetch_lines_scoped
)
from PF_LU_APP.traps.bait_station_repository import (
    retire_bait_station, reactivate_bait_station,
    fetch_bait_station_details_scoped, update_bait_station_full
)

@admin_bp.route('/edit_bait_station/<string:bait_station_code>', methods=['GET', 'POST'])
@roles_required(ROLE_SUPER_ADMIN, ROLE_COORDINATOR)
def edit_bait_station(bait_station_code):
    conn = get_db()
    try:
        with get_cursor_context() as cursor:
            bait_station = fetch_bait_station_details_scoped(
                cursor, bait_station_code, session.get('current_group_id'), session.get('is_super_admin')
            )

            if not bait_station:
                flash("Bait station not found or access denied.", "danger")
                return redirect(url_for('admin.view_lines'))

            is_retired = (bait_station.get('status') == 'inactive' or bait_station.get('equipment_status_name') == 'Retired')

            if request.method == 'POST':
                if is_retired:
                    flash("The item is no longer in service.", "danger")
                    return redirect(url_for('admin.view_lines'))

                bait_station_type_id = request.form.get('bait_station_type_id')
                line_id = request.form.get('line_id')
                storage_area_id = request.form.get('storage_area_id')
                latitude = request.form.get('latitude')
                longitude = request.form.get('longitude')
                equipment_status_name = request.form.get('equipment_status')

                if not bait_station_type_id or not equipment_status_name or not (line_id or storage_area_id):
                    flash("Please fill out all required fields and choose either a line or a storage area.", "danger")
                    return redirect(url_for('admin.edit_bait_station', bait_station_code=bait_station_code))

                if line_id and storage_area_id:
                    flash("Please select either a line or a storage area, not both.", "danger")
                    return redirect(url_for('admin.edit_bait_station', bait_station_code=bait_station_code))

                if equipment_status_name == 'In Storage' and not storage_area_id:
                    flash("Please select a storage location when moving a bait station into storage.", "danger")
                    return redirect(url_for('admin.edit_bait_station', bait_station_code=bait_station_code))

                if equipment_status_name == 'In Storage' and line_id:
                    flash("When moving a bait station into storage, clear the line selection and choose a storage area.", "danger")
                    return redirect(url_for('admin.edit_bait_station', bait_station_code=bait_station_code))

                l_id = None
                sa_id = None
                lat = None
                lng = None

                if line_id:
                    cursor.execute("SELECT group_id FROM lines WHERE line_id = %s", (line_id,))
                    new_line = cursor.fetchone()
                    if not new_line or (not session.get('is_super_admin') and str(new_line['group_id']) != str(session.get('current_group_id'))):
                        flash("Unauthorized line assignment.", "danger")
                        return redirect(url_for('admin.edit_bait_station', bait_station_code=bait_station_code))
                    try:
                        l_id = int(line_id)
                        lat = float(latitude)
                        lng = float(longitude)
                    except (ValueError, TypeError):
                        flash("Valid line coordinates and station type are required.", "danger")
                        return redirect(url_for('admin.edit_bait_station', bait_station_code=bait_station_code))

                if storage_area_id:
                    cursor.execute("SELECT group_id FROM storage_area WHERE storage_area_id = %s", (storage_area_id,))
                    storage_area = cursor.fetchone()
                    if not storage_area or (not session.get('is_super_admin') and str(storage_area['group_id']) != str(session.get('current_group_id'))):
                        flash("Unauthorized storage assignment.", "danger")
                        return redirect(url_for('admin.edit_bait_station', bait_station_code=bait_station_code))
                    try:
                        sa_id = int(storage_area_id)
                    except (ValueError, TypeError):
                        flash("Invalid storage area selected.", "danger")
                        return redirect(url_for('admin.edit_bait_station', bait_station_code=bait_station_code))

                try:
                    bs_type_id = int(bait_station_type_id)
                except (ValueError, TypeError):
                    flash("Invalid bait station type selected.", "danger")
                    return redirect(url_for('admin.edit_bait_station', bait_station_code=bait_station_code))

                cursor.execute("SELECT group_id FROM lines WHERE line_id = %s", (l_id,))
                line_row = cursor.fetchone()
                line_group_id = line_row['group_id'] if line_row else session.get('current_group_id')

                if not validate_location_within_boundary(cursor, line_group_id, lat, lng):
                    flash("Cannot update bait station: coordinates are outside the group boundary.", "danger")
                    return redirect(url_for('admin.edit_bait_station', bait_station_code=bait_station_code))

                updated_status = 'inactive' if equipment_status_name == 'Retired' else 'active'
                equipment_status_id = get_equipment_status_id(cursor, equipment_status_name)
                update_bait_station_full(cursor, bait_station_code, bs_type_id, l_id, sa_id, lat, lng, equipment_status_id, updated_status)
                conn.commit()
                flash("Bait station updated successfully!", "success")
                return redirect(url_for('admin.view_lines'))

            lines = fetch_lines_scoped(cursor, session.get('current_group_id'), session.get('is_super_admin'))
            storage_areas = fetch_storage_areas_scoped(cursor, session.get('current_group_id'), session.get('is_super_admin'))
            cursor.execute('SELECT * FROM bait_station_type ORDER BY bait_station_type_name ASC')
            bait_station_types = cursor.fetchall()
            for status_name in ['Active', 'In Storage', 'Under Repair', 'Retired']:
                get_equipment_status_id(cursor, status_name)
            cursor.execute('SELECT * FROM equipment_status ORDER BY equipment_status_name ASC')
            equipment_statuses = cursor.fetchall()

            return render_template('admin/edit_bait_station.html', bait_station=bait_station, lines=lines, storage_areas=storage_areas, bait_station_types=bait_station_types, equipment_statuses=equipment_statuses, is_retired=is_retired)
    except Exception as e:
        current_app.logger.exception(f"Error editing bait station: {e}")
        flash("An error occurred while editing the bait station.", "danger")
        return redirect(url_for('admin.view_lines'))
    except Exception as e:
        current_app.logger.exception(f"Error editing bait station: {e}")
        flash("An error occurred while editing the bait station.", "danger")
        return redirect(url_for('admin.view_lines'))

@admin_bp.route('/action_retire_bait_station/<string:bait_station_code>', methods=['POST'])
@roles_required(ROLE_SUPER_ADMIN, ROLE_COORDINATOR)
def action_retire_bait_station(bait_station_code):
    try:
        conn = get_db()
        with get_cursor_context() as cursor:
            cursor.execute("SELECT l.group_id FROM bait_stations b JOIN lines l ON b.line_id = l.line_id WHERE b.bait_station_code = %s", (bait_station_code,))
            res = cursor.fetchone()
            
            if not res or (not session.get('is_super_admin') and str(res['group_id']) != str(session.get('current_group_id'))):
                flash("Access denied.", "danger")
                return redirect(url_for('admin.view_lines'))
            
            retired_id = get_equipment_status_id(cursor, 'Retired')
            
            cursor.execute("UPDATE bait_stations SET status = 'inactive', equipment_status_id = %s WHERE bait_station_code = %s", (retired_id, bait_station_code))
            conn.commit()
            flash(f"Bait station {bait_station_code} was successfully retired.", "success")
    except Exception as e:
        current_app.logger.exception(f"Error retiring bait station: {e}")
        flash(f"Error retiring bait station: {e}", "danger")
    return redirect(url_for('admin.view_lines'))

@admin_bp.route('/action_restore_bait_station/<string:bait_station_code>', methods=['POST'])
@roles_required(ROLE_SUPER_ADMIN, ROLE_COORDINATOR)
def action_restore_bait_station(bait_station_code):
    try:
        conn = get_db()
        with get_cursor_context() as cursor:
            cursor.execute("UPDATE bait_stations SET status = 'active' WHERE bait_station_code = %s", (bait_station_code,))
            conn.commit()
            flash(f"Bait station {bait_station_code} was successfully restored.", "success")
    except Exception as e:
        current_app.logger.exception(f"Error restoring bait station: {e}")
        flash("An error occurred while restoring the bait station.", "danger")
    return redirect(url_for('admin.retired_assets'))
