...
@operator_bp.route('/graphs', methods=['GET', 'POST'])
...
def data_graphs():
    ...
    try:
        ...
        return render_template('dashboards/graphs.html',
                               ...
                               station_hotspots=[], role='operator')

@operator_bp.route('/operator/download_csv')
@roles_required(ROLE_SUPER_ADMIN, ROLE_COORDINATOR, ROLE_OPERATOR)
def download_catches_csv():
    ...
    writer.writerow(['Record ID', 'Trap Code', 'Date', 'Line', 'Species', 'Sex', 'Maturity', 'Trap Status', 'Rebaited', 'Bait Type', 'Trap Condition', 'Strikes', 'Note', 'Recorded By'])
    for c in catches:
        writer.writerow([c['catches_id'], c['trap_code'], c['date'].strftime('%Y-%m-%d %H:%M') if c['date'] else '', c['line_name'], c['species_name'], c['sex'], c['maturity'], c['trap_status'], 'Yes' if c['rebaited'] else 'No', c['bait_type_name'] or '', c['trap_condition'], c['strikes'], c['note'], c['recorded_by_name']])
    
    response = make_response(output.getvalue())
    response.headers['Content-Disposition'] = 'attachment; filename=operator_catch_records.csv'
    response.headers['Content-Type'] = 'text/csv; charset=utf-8'
    return response

@operator_bp.route('/graphs/download-report', methods=['GET'])
@roles_required(ROLE_SUPER_ADMIN, ROLE_COORDINATOR)
def download_graphs_report():
    if 'user_id' not in session or session.get('role_id') not in (ROLE_SUPER_ADMIN, ROLE_COORDINATOR):
    ...
    try:
        ...
        return redirect(url_for('operator.data_graphs', line_id=line_id, start_date=start_date, end_date=end_date))

@operator_bp.route('/trend-analytics', methods=['GET'])
@roles_required(ROLE_SUPER_ADMIN, ROLE_COORDINATOR, ROLE_OPERATOR)
def trend_analytics():
